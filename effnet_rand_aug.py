...
def _convert_angles_to_transform(angles: tf.Tensor, image_width: tf.Tensor, image_height: tf.Tensor) -> tf.Tensor:
    ...
    x_offset = ((image_width - 1) - (
                tf.math.cos(angles) * (image_width - 1) - tf.math.sin(angles) * (image_height - 1))) / 2.0
    y_offset = ((image_height - 1) - (
                tf.math.sin(angles) * (image_width - 1) + tf.math.cos(angles) * (image_height - 1))) / 2.0
...
def transform(image: tf.Tensor, transforms) -> tf.Tensor:
    ...
    image = to_4d(image)
    image = transform_og(images=image, transforms=transforms, interpolation="nearest")
...
def unwrap(image: tf.Tensor, replace: int) -> tf.Tensor:
    ...
    flattened_image = tf.where(tf.equal(alpha_channel, 0), tf.ones_like(flattened_image, dtype=image.dtype) * replace,
                               flattened_image)
...
def select_and_apply_random_policy(policies: Any, image: tf.Tensor):
    ...
    for (i, policy) in enumerate(policies):
        image = tf.cond(tf.equal(i, policy_to_select), lambda selected_policy=policy: selected_policy(image),
                        lambda: image)
    return image
...
def _parse_policy_info(name: Text, prob: float, level: float, replace_value: List[int], cutout_const: float,
...
class RandAugment(ImageAugment):
    ...
    def __init__(self, num_layers: int = 2, magnitude: float = 10.0, cutout_const: float = 40.0,
    ...
    def distort(self, image: tf.Tensor) -> tf.Tensor:
        ...
        for _ in range(self.num_layers):
            ...
            for (i, op_name) in enumerate(self.available_ops):
                ...
                func, _, args = _parse_policy_info(op_name, prob, self.magnitude, replace_value, self.cutout_const,
...
class RandomProcessImage2:
    def __init__(self, target_shape=(300, 300), magnitude=0, keep_shape=False):
        ...
        if magnitude > 0:
            ...
            print(">>>> RandAugment: magnitude = %d, translate_const = %d, cutout_const = %d" % (
            magnitude, translate_const, cutout_const))
            aa = RandAugment(magnitude=magnitude, translate_const=translate_const, cutout_const=cutout_const)
    ...
    def __call__(self, datapoint, label):
...
FILE_HASH_DICT = {
    "b0": {"21k-ft1k": "4e4da4eb629897e4d6271e131039fe75", "21k": "5dbb4252df24b931e74cdd94d150f25a",
           "imagenet": "9abdc43cb00f4cb06a8bdae881f412d6"},
    "b1": {"21k-ft1k": "5f1aee82209f4f0f20bd24460270564e", "21k": "a50ae65b50ceff7f5283be2f4506d2c2",
           "imagenet": "5d4223b59ff268828d5112a1630e234e"},
    "b2": {"21k-ft1k": "ec384b84441ddf6419938d1e5a0cbef2", "21k": "9f718a8bbb7b63c5313916c5e504790d",
           "imagenet": "1814bc08d4bb7a5e0ed3ccfe1cf18650"},
    "b3": {"21k-ft1k": "4a27827b0b2df508bed31ae231003bb1", "21k": "ade5bdbbdf1d54c4561aa41511525855",
           "imagenet": "cda85b8494c7ec5a68dffb335a254bab"},
    "l": {"21k-ft1k": "30327edcf1390d10e9a0de42a2d731e3", "21k": "7970f913eec1b4918e007c8580726412",
          "imagenet": "2b65f5789f4d2f1bf66ecd6d9c5c2d46"},
    "m": {"21k-ft1k": "0c236c3020e3857de1e5f2939abd0cc6", "21k": "3923c286366b2a5137f39d1e5b14e202",
          "imagenet": "ac3fd0ff91b35d18d1df8f1895efe1d5"},
    "s": {"21k-ft1k": "93046a0d601da46bfce9d4ca14224c83", "21k": "10b05d878b64f796ab984a5316a4a1c3",
...
def MBConv(inputs, output_channel, stride, expand_ratio, shortcut, kernel_size=3, drop_rate=0, use_se=0, is_fused=False,
           is_torch_mode=False, name=""):
    channel_axis = 1 if K.image_data_format() == "channels_first" else -1
    input_channel = inputs.shape[channel_axis]

    if is_fused and expand_ratio != 1:
        nn = conv2d_no_bias(inputs, input_channel * expand_ratio, (3, 3), stride, padding="same",
                            use_torch_padding=is_torch_mode, name=name + "sortcut_")
        nn = batchnorm_with_activation(nn, use_torch_eps=is_torch_mode, name=name + "sortcut_")
    elif expand_ratio != 1:
        nn = conv2d_no_bias(inputs, input_channel * expand_ratio, (1, 1), strides=(1, 1), padding="valid",
                            name=name + "sortcut_")
        nn = batchnorm_with_activation(nn, use_torch_eps=is_torch_mode, name=name + "sortcut_")
    else:
        nn = inputs
    ...
    if not is_fused:
        if is_torch_mode and kernel_size // 2 > 0:
            ...
            pad = "SAME"
        nn = DepthwiseConv2D(kernel_size, padding=pad, strides=stride, use_bias=False,
                             depthwise_initializer=CONV_KERNEL_INITIALIZER, name=name + "MB_dw_")(nn)
    ...
    # pw-linear
    if is_fused and expand_ratio == 1:
        nn = conv2d_no_bias(nn, output_channel, (3, 3), strides=stride, padding="same", use_torch_padding=is_torch_mode,
                            name=name + "fu_")
...
def EfficientNetV2(
    ...
    rescale_mode = "tf" if pretrained is not None and pretrained.startswith("imagenet21k") else blocks_config.get(
        "rescale_mode", "torch")

    inputs = Input(shape=input_shape)
    if include_preprocessing and rescale_mode == "torch":
        channel_axis = 1 if K.image_data_format() == "channels_first" else -1
        Normalization = keras.layers.Normalization if hasattr(keras.layers,
                                                              "Normalization") else keras.layers.experimental.preprocessing.Normalization
        ...
        Rescaling = keras.layers.Rescaling if hasattr(keras.layers,
                                                      "Rescaling") else keras.layers.experimental.preprocessing.Rescaling
        nn = Rescaling(scale=1.0 / 128.0, offset=-1)(inputs)
    else:
        nn = inputs
    out_channel = _make_divisible(first_conv_filter, 8)
    nn = conv2d_no_bias(nn, out_channel, (3, 3), strides=first_strides, padding="same", use_torch_padding=is_torch_mode,
                        name="stem_")
    ...
    for id, (expand, out_channel, depth, stride, se, kernel_size) in enumerate(
        ...
        for block_id in range(depth):
            ...
            nn = MBConv(nn, out, stride, expand, shortcut, kernel_size, block_drop_rate, se, is_fused, is_torch_mode,
                        name=name)
            pre_out = out
            global_block_id += 1
...
def EfficientNetV2S(input_shape=(384, 384, 3), num_classes=1000, dropout=0.2,
